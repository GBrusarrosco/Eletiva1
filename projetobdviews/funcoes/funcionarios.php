...
function listarPontos(int $funcionario_id): array {
    global $pdo;
    ...
    $stmt = $pdo->prepare("
        SELECT 
            ...
            t.periodo AS turno
        FROM batida_ponto b
        JOIN funcionario f ON b.funcionario_id = f.id
        JOIN cargo c ON f.cargo_id = c.id
        JOIN turno t ON f.turno_id = t.id
        WHERE b.funcionario_id = :funcionario_id
...
function listarPontosPorMes(int $funcionario_id): array {
    global $pdo;
    ...
    $stmt = $pdo->prepare("
        ...
        FROM batida_ponto b
        WHERE b.funcionario_id = :funcionario_id
        AND b.data BETWEEN :inicio_mes AND :fim_mes
    ");
    $stmt->execute([
        ':funcionario_id' => $funcionario_id,
    ...
    return ['total_pontos' => $resultado['total_pontos'] ?? 0];
}
